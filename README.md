...
To get a grasp on the generality of this mechanism, picture this. Take a trainable (N, N) matrix, whose entries are $P(i, j)$ (really $c^{\top}(p_{1,j}+p_{2,i})$ ), and sum to it the similarity of $x_j$ with a trainable vector of the same dimension every row, and the same across every column with a different trainable vector, and the same again across the diagonal. Then, perform standard softmax across rows, and use this as an (N, N) weight. 
